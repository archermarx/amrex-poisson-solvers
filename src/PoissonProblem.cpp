...
void PoissonProblem::solve() {
    ...
    // Additionally, it also provides boundary conditions for Dirichlet boundaries if there are any.
    for (int ilev = 0; ilev <= max_level; ++ilev) {
        MultiFab::Copy(solution[ilev], exact_solution[ilev], 0, 0, 1, 0);
        const Box &interior = surroundingNodes(
            amrex::grow(geom[ilev].Domain(), -1)
        );

        // Usually we want the best initial guess. For testing here, we set the domain boundaries to the
        // exact solution and zero out the interior
...
void PoissonProblem::read_parameters() {
    ParmParse pp;
